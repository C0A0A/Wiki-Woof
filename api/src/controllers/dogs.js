...
const apiDogsBkp = require('../resources/breed_list.json')
	...
	Promise.all([
		...
		.then((dogs) => {
			let dbDogs = dogs[0].length
				...
				: [];
			let apiDogs = apiDogsBkp.map((dog) => {
				return {
					id: dog.id,
					name: dog.name,
			...
			if (dogsResult.length < 8 || dogsResult.length < standarLimit)
				return res.send({
					dogs: dogsResult,
					previousUrl: `${process.env.BACKEND_URL
						}/dogs/?filter=${filter}&filtervalue=${filterValue}&orderby=${order[0]
						}&direction=${order[1]}&offset0=${standarOffset - standarLimit
						}&limit=${standarLimit}`,
					nextUrl: null,
				});
			const nextUrl = `${process.env.BACKEND_URL
				}/dogs/?filter=${filter}&filtervalue=${filterValue}&orderby=${order[0]
				}&direction=${order[1]}&offset0=${standarOffset + standarLimit
				}&limit=${standarLimit}`;
