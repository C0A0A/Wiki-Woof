...
function getSomeDogs (req, res) {
    ...
    const order = req.query.orderby && !req.query.direction ? [req.query.orderby, "ASC"] : req.query.orderby && req.query.direction ? [req.query.orderby, req.query.direction] : ["id", "ASC"];
   ...
   Promise.all([Dog.findAll({where: where, include: [{model: Image, as: "image", attributes: ["urlImage"]}, {model: Temperament, as: "temperaments", attributes:["name"], through: {attributes: []}}] , order: [order]}), axios.get(`${RAZASURL}?${API_KEY}`)])
   .then(dogs => {
    let dbDogs =  dogs[0].length ? dogs[0].map(dog => dog.dataValues).map(dog => {
    ...
    let apiDogs = dogs[1].data.length ? dogs[1].data.map(dog => { 
    ...
    let dogsResult = bubbleSort(apiDogs.concat(dbDogs), order[0], order[1]).splice(standarOffset, standarLimit);
    ...
    if(dogsResult.length < 8 || dogsResult.length < standarLimit) return res.send({ 
        dogs: dogsResult,
        previousUrl: `http://localhost:3001/dogs/?filter=${filter}&filtervalue=${filterValue}&orderby=${order[0]}&direction=${order[1]}&offset0=${standarOffset - standarLimit}&limit=${standarLimit}`,
        nextUrl: null
    });
    const nextUrl = `http://localhost:3001/dogs/?filter=${filter}&filtervalue=${filterValue}&orderby=${order[0]}&direction=${order[1]}&offset0=${standarOffset + standarLimit}&limit=${standarLimit}`;
    const previousUrl = !standarOffset ? null : `http://localhost:3001/dogs/?filter=${filter}&filtervalue=${filterValue}&orderby=${order[0]}&direction=${order[1]}&offset0=${standarOffset - standarLimit}&limit=${standarLimit}`;
    return res.send({
        dogs: dogsResult,
        nextUrl,
        previousUrl
...
function addBreed (req, res) {
    ...
    const temperaments = req.body.temperament && req.body.temperament.split(",").map(item => item.trim());
    const file = req.file
    let itemsCreated = [];
...
function findById (req, res) {
    if(req.params.id.includes("-")) {
        ...
        .then(dog => {
            if(!dog) return res.send({type: "Not found", error: "The id you are requesting does not exist"});
    ...
    .then(dogs => {
        ...
        return res.send({type: "Not found", error: "The id you are requesting does not exist"});
    })
    .catch(err => res.status(500).send({type: "Internal Server Error.",  error: err.message}));
}}
...
function getDataBaseDogs (req, res) {
    ...
    const filterValue = req.query.filtervalue ? req.query.filtervalue : "";
    const where = filter === "name" ? {name : {[Op.like]:`%${filterValue}%`}} : filter === "weight" ? {weight : {[Op.like]:`%${filterValue}%`}} : filter === "height" ? {height : {[Op.like]:`%${filterValue}%`}} : filter === "life_span" ? {life_span : {[Op.like]:`%${filterValue}%`}} : filter === "image_url" ? {"$image.urlImage$": { [Op.like]: `%${filterValue}%` }} : filter === "temperament" ? {"$temperaments.name$": { [Op.like]: `%${filterValue}%` }} : {}
    Dog.findAll({where: where, include: [{model: Image, as: "image", attributes: ["urlImage"]}, {model: Temperament, as: "temperaments", attributes:["name"], through: {attributes: []}}]})
    .then(data => {
        if(!data.length) return res.send({type: "Not found", error: "The query does not return results"});
        let result = data.map(dog => dog.dataValues).map(dog => {
            return {id: dog.id, name: dog.name, weight: dog.weight, height: dog.height, life_span: dog.life_span, image_url: dog.image && dog.image.urlImage, temperament: dog.temperaments.map(tem => tem.name).join(", ")}
        });
